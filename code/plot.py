...
def fig4(input_file1,input_file2,input_file3,input_file4,input_file5,input_file6,output_file):
    ...
    df5 = po.read_data(input_file5)
    df6 = po.read_data(input_file6)
    ...
    time5 = df5['Time(s)'] * 1e9;
    time6 = df6['Time(s)'] * 1e9;
    ...
    wks.from_df(pd.DataFrame({
        ...
        'Time(ns)5': time5,           
        'Voltage(V)5': df5['Voltage(V)'],    
        'Current(A)5': df5['Current(A)'],

        'Time(ns)6': time6,           
        'Voltage(V)6': df6['Voltage(V)'],    
        'Current(A)6': df6['Current(A)'],
    ...
    po.plot_set(wks, lay, colx='Time(ns)5', coly='Voltage(V)5', color="#38EB14", width=3, type='l')

    po.plot_set(wks, lay, colx='Time(ns)6', coly='Voltage(V)6', color="#D6EB14", width=3, type='l')
...
def fig6(input_file1,input_file2,input_file3,input_file4,input_file5,output_file):
     # 读取数据
    df1 = po.read_data(input_file1)
    df2 = po.read_data(input_file2)
    df3 = po.read_data(input_file3)
    ...
    #po.plot_set(wks, lay, colx='Time(ns)1', coly='Voltage(V)1', color='black', width=3, type='l')

    po.plot_set(wks, lay, colx='Time(ns)2', coly='Voltage(V)2', color='blue', width=3, type='l')

    po.plot_set(wks, lay, colx='Time(ns)3', coly='Voltage(V)3', color="#EB7114", width=3, type='l')

    po.plot_set(wks, lay, colx='Time(ns)4', coly='Voltage(V)4', color="red", width=3, type='l')

    po.plot_set(wks, lay, colx='Time(ns)5', coly='Voltage(V)5', color="#EBD614", width=3, type='l')

    
    po.lay_set(gp, lay, po.LayConfig(
        x = po.AxisConfig(title='Time/ns',begin=-0.5,end=10,step=2),
        y = po.AxisConfig(title='Voltage/V',end=100,step=20),
        legend=po.TextConfig(title=['0.3ns','0.6ns','1.0ns','1.2ns'])
    ))
    #------------------------参数设置------------------------
    print("绘图完成")
